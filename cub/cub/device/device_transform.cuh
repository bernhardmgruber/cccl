...
struct DeviceTransform
{
  ...
  CUB_RUNTIME_FUNCTION static cudaError_t ManyToOne(
    int count,
  ...
  {
    CUB_DETAIL_NVTX_RANGE_SCOPE("cub::DeviceTransform::TransformMany");
  ...
  /// Like \ref ManyToOne, but the objects passed to the call operator of the transformation operation are guaranteed to
  /// reside in the input streams and are never copied.
  template <typename... RandomAccessIteratorsIn, typename RandomAccessIteratorOut, typename TransformOp>
  CUB_RUNTIME_FUNCTION static cudaError_t ManyToOneStableParameterAddresses(
    int count,
    ::cuda::std::tuple<RandomAccessIteratorsIn...> inputs,
    RandomAccessIteratorOut output,
    TransformOp transform_op,
    ...
    cudaStream_t stream = nullptr)
  {
    ...
    return ManyToOneStableParameterAddresses(
      count, ::cuda::std::move(inputs), ::cuda::std::move(output), ::cuda::std::move(transform_op), stream);
  }
};
