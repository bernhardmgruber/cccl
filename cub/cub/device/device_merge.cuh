...
struct DeviceMerge
{
  //! @brief Merges two sorted sequences of values (called keys) into a sorted output sequence. Merging is unstable,
  ...
  //! @par
  //! @code
  //! .. literalinclude:: ../../test/catch2_test_device_merge_api.cu
  //!     :language: c++
  //!     :dedent:
  ...
  //! @tparam Offset **[deduced]** Integral used for size and offset calculations.
  ...
  template <typename KeyIteratorIn1,
            ...
            typename Offset,
            typename CompareOp = ::cuda::std::less<typename ::cuda::std::iterator_traits<
              KeyIteratorIn1>::value_type>> // TODO(bgruber): use transparent comparison operator in C++14
    ...
    cudaStream_t stream  = 0)
  {
    ...
    return detail::
      DispatchMerge<KeyIteratorIn1, NullType*, KeyIteratorIn2, NullType*, KeyIteratorOut, NullType*, Offset, CompareOp>::
        Dispatch(
          d_temp_storage,
          temp_storage_bytes,
          keys_in1,
  ...
  //! @brief Merges two sorted sequences of key-value pairs into a sorted output sequence. Merging is unstable,
  //! which means any two equivalent values (neither value is ordered before the other) may be written to the ouput
  //! sequence in any order.
  //!
  //! @par Snippet
