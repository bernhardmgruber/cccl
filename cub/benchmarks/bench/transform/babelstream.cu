...
#else
  using policy_t = cub::detail::transform::policy_hub<RandomAccessIteratorsInTuple>;
#endif
...
static void heavy(nvbench::state& state)
{
  ...
  state.add_global_memory_reads<T>(n);
  state.add_global_memory_writes<T>(n);
  bench_transform(state, n, ::cuda::std::tuple{in.begin()}, out.begin(), [=] _CCCL_DEVICE(int data) {
    int reg[heavyness];
    reg[0] = data;
