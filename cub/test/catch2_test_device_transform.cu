...
struct policy_hub_for_alg
{
  struct max_policy : cub::ChainedPolicy<300, max_policy, max_policy>
  {
    ...
    static constexpr Algorithm alg_addr_unstable = Alg;
    static constexpr Algorithm alg_addr_stable   = Alg;
    using prefetch_policy                        = cub::detail::transform::prefetch_policy_t<256>;
    using unrolled_policy                        = cub::detail::transform::unrolled_policy_t<256, 4>;
    using ublkcp_policy                          = cub::detail::transform::async_copy_policy_t<256>;
  };
};
  ...
  cudaStream_t stream = nullptr)
{
  ...
  return cub::detail::transform::dispatch_t<CanCopy,
                                            Offset,
                                            ::cuda::std::tuple<RandomAccessIteratorsIn...>,
                                            RandomAccessIteartorOut,
                                            TransformOp,
                                            policy_hub_for_alg<Alg>>{}
    .dispatch(num_items, inputs, output, transform_op, stream);
}

DECLARE_LAUNCH_WRAPPER(cub::DeviceTransform::ManyToOne, transform_many);
DECLARE_LAUNCH_WRAPPER(cub::DeviceTransform::ManyToOneStableParameterAddresses, transform_many_stable);
DECLARE_TMPL_LAUNCH_WRAPPER(transform_many_with_alg_entry_point,
                            transform_many_with_alg,
                            ESCAPE_LIST(Algorithm Alg, bool CanCopy, typename Offset),
                            ESCAPE_LIST(Alg, CanCopy, Offset));
...
CUB_TEST("DeviceTransform::ManyToOne BabelStream add",
         ...
         algorithms)
{
  ...
  transform_many_with_alg<alg, true, offset_t>(
    num_items, ::cuda::std::make_tuple(a.begin(), b.begin()), result.begin(), ::cuda::std::plus<type>{});
...
CUB_TEST("DeviceTransform::ManyToOne BabelStream nstream",
         "[device][device_transform]",
         c2h::type_list<std::uint8_t, std::uint16_t, std::uint32_t, std::uint64_t>,
         offset_types,
         algorithms)
{
  ...
  transform_many_with_alg<alg, true, offset_t>(
    num_items, ::cuda::std::make_tuple(a.begin(), b.begin(), c.begin()), a.begin(), nstream_kernel<type>{});
...
CUB_TEST("DeviceTransform::ManyToOne add five streams", "[device][device_transform]", algorithms)
{
  using offset_t     = int;
  ...
  transform_many_with_alg<alg, true, offset_t>(
    num_items, ::cuda::std::make_tuple(a.begin(), b.begin(), c.begin(), d.begin(), e.begin()), result.begin(), Sum{});
...
CUB_TEST("DeviceTransform::ManyToOne no streams", "[device][device_transform]")
{
  constexpr int num_items = 100;
  c2h::device_vector<int> result(num_items);
  transform_many(num_items, ::cuda::std::tuple<>{}, result.begin(), GiveMeFive{});
...
CUB_TEST("DeviceTransform::ManyToOne fancy input iterator types", "[device][device_transform]", algorithms)
{
  using type         = int;
  using offset_t     = int;
  constexpr auto alg = c2h::get<0, TestType>::value;
  ...
  c2h::device_vector<type> result(num_items);
  transform_many_with_alg<alg, true, offset_t>(
    num_items, ::cuda::std::make_tuple(a, b), result.begin(), ::cuda::std::plus<type>{});
...
CUB_TEST("DeviceTransform::ManyToOne fancy output iterator type", "[device][device_transform]", algorithms)
{
  using type         = int;
  using offset_t     = int;
  constexpr auto alg = c2h::get<0, TestType>::value;
  ...
  transform_many_with_alg<alg, true, offset_t>(
    num_items, ::cuda::std::make_tuple(a.begin(), b.end()), thrust::discard_iterator<>{}, ::cuda::std::plus<type>{});
}

CUB_TEST("DeviceTransform::ManyToOne mixed input iterator types", "[device][device_transform]", algorithms)
{
  using type         = int;
  using offset_t     = int;
  constexpr auto alg = c2h::get<0, TestType>::value;
  ...
  c2h::device_vector<type> result(num_items);
  transform_many_with_alg<alg, true, offset_t>(
    num_items, ::cuda::std::make_tuple(a, b.begin()), result.begin(), ::cuda::std::plus<type>{});
...
CUB_TEST("DeviceTransform::ManyToOne address stability", "[device][device_transform]")
{
  using type = int;
...
CUB_TEST("DeviceTransform::ManyToOne not trivially relocatable", "[device][device_transform]")
{
  constexpr int num_items = 100;
  c2h::device_vector<non_trivial> input(num_items, non_trivial{42});
  c2h::device_vector<non_trivial> result(num_items);
  transform_many(num_items, ::cuda::std::make_tuple(input.begin()), result.begin(), ::cuda::std::negate<>{});

  const auto reference = c2h::device_vector<non_trivial>(num_items, non_trivial{-42});
  REQUIRE((reference == result));
}

CUB_TEST("DeviceTransform::ManyToOne buffer start alignment",
         "[device][device_transform]",
         c2h::type_list<std::uint8_t, std::uint16_t, float, double>)
{
  ...
  c2h::device_vector<type> result(num_items);
  transform_many(
    num_items, ::cuda::std::make_tuple(input.begin() + offset), result.begin() + offset, ::cuda::std::negate<>{});
...
{
  _CCCL_HOST_DEVICE int operator()(int data) const
  {
    int reg[N];
    reg[0] = data;
    for (int i = 1; i < N; ++i)
...
CUB_TEST("DeviceTransform::ManyToOne heavy functor",
         "[device][device_transform]",
         algorithms,
         c2h::enum_type_list<int, 32, 64, 128, 256>)
{
  ...
  constexpr int num_items = 100;
  c2h::device_vector<int> input(num_items);
  c2h::gen(CUB_SEED(1), input, 1, 10);
  c2h::device_vector<int> result(num_items);
  transform_many_with_alg<alg, true, offset_t>(
    num_items, ::cuda::std::make_tuple(input.begin()), result.begin(), heavy_functor<heavyness>{});

  // compute reference and verify
  c2h::host_vector<int> input_h = input;
  c2h::host_vector<int> reference_h(num_items);
