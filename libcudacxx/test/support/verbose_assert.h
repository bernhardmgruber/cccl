...
template <
  class Tp,
  int ST = (IsStreamable<decltype(std::cerr), Tp>::value ? 1 : (IsStreamable<decltype(std::wcerr), Tp>::value ? 2 : -1))>
struct SelectStream
{
  static_assert(ST == -1, "specialization required for ST != -1");
  static void Print(Tp const&)
  {
    std::clog << "Value Not Streamable!\n";
  }
};
...
{
  static void Print(Tp const& val)
  {
    std::cerr << val;
  }
};
...
struct AssertData
{
  AssertData(const char* xcheck, const char* xfile, const char* xfunc, unsigned long xline, bool xpassed = true)
      : passed(xpassed)
      , check(xcheck)
      , file(xfile)
      , func(xfunc)
...
AssertData CheckCollectionsEqual(It1 F1, It1 E1, It2 F2, It2 E2, AssertData Data, Comp C = EqualToComp())
{
  const It1 F1Orig = F1;
  const It2 F2Orig = F2;
  bool Failed      = false;
...
#define ASSERT(...)                                                                                  \
  ::verbose_assert::AssertHandler(                                                                   \
    ::verbose_assert::AssertData(#__VA_ARGS__, __FILE__, ASSERT_FN_NAME(), __LINE__, (__VA_ARGS__))) \
    .GetLog()
...
#define ASSERT_PRED(PRED, LHS, RHS) ASSERT(PRED(LHS, RHS)) << DISPLAY(LHS) << DISPLAY(RHS)
...
#define ASSERT_COLLECTION_EQ_COMP(F1, E1, F2, E2, Comp)                                                      \
  (::verbose_assert::AssertHandler(                                                                          \
     ::verbose_assert::CheckCollectionsEqual(                                                                \
       ...
       F2,                                                                                                   \
       E2,                                                                                                   \
       ::verbose_assert::AssertData(                                                                         \
         "CheckCollectionsEqual(" #F1 ", " #E1 ", " #F2 ", " #E2 ")", __FILE__, ASSERT_FN_NAME(), __LINE__), \
       Comp))                                                                                                \
     .GetLog())

#define ASSERT_COLLECTION_EQ(F1, E1, F2, E2) ASSERT_COLLECTION_EQ_COMP(F1, E1, F2, E2, ::verbose_assert::EqualToComp())
