...
//                         TEST_SUITE_END
////////////////////////////////////////////////////////////////////////////////
#define TEST_SUITE_END() } /* namespace opened in TEST_SUITE(...) */
#
...
#define TEST_WARN(...)                                                                                                \
  ...
  {                                                                                                                   \
    TEST_SET_CHECKPOINT();                                                                                            \
    ::rapid_cxx_test::test_outcome m_f(                                                                               \
      ::rapid_cxx_test::failure_type::none, __FILE__, TEST_FUNC_NAME(), __LINE__, "TEST_WARN(" #__VA_ARGS__ ")", ""); \
    if (not(__VA_ARGS__))                                                                                             \
    {                                                                                                                 \
...
#define TEST_CHECK(...)                                                                                                \
  ...
  {                                                                                                                    \
    TEST_SET_CHECKPOINT();                                                                                             \
    ::rapid_cxx_test::test_outcome m_f(                                                                                \
      ::rapid_cxx_test::failure_type::none, __FILE__, TEST_FUNC_NAME(), __LINE__, "TEST_CHECK(" #__VA_ARGS__ ")", ""); \
    if (not(__VA_ARGS__))                                                                                              \
    {                                                                                                                  \
...
#define TEST_ASSERT(...)                                                                                                \
  ...
  {                                                                                                                     \
    TEST_SET_CHECKPOINT();                                                                                              \
    ::rapid_cxx_test::test_outcome m_f(                                                                                 \
      ::rapid_cxx_test::failure_type::none, __FILE__, TEST_FUNC_NAME(), __LINE__, "TEST_ASSERT(" #__VA_ARGS__ ")", ""); \
    if (not(__VA_ARGS__))                                                                                               \
    {                                                                                                                   \
...
#  define TEST_CHECK_THROW_RESULT(Except, Checker, ...)                       \
    ...
    {                                                                         \
      ...
      ::rapid_cxx_test::test_outcome m_f(                                     \
        ...
        TEST_FUNC_NAME(),                                                     \
        __LINE__,                                                             \
        "TEST_CHECK_THROW_RESULT(" #Except "," #Checker "," #__VA_ARGS__ ")", \
        "");                                                                  \
      try                                                                     \
      {                                                                       \
        (static_cast<void>(__VA_ARGS__));                                     \
...
struct test_outcome
{
  ...
  test_outcome(failure_type_t type1,
               const char* file1,
               const char* func1,
               std::size_t line1,
...
public:
  test_reporter()
      : m_testcases(0)
      , m_testcase_failures(0)
      , m_unsupported(0)
      , m_assertions(0)
      , m_warning_failures(0)
      , m_check_failures(0)
      , m_require_failures(0)
      , m_uncaught_exceptions(0)
      , m_failure()
  {}
  ...
  void test_case_end()
  {
    if (m_failure.type != failure_type::none && m_failure.type != failure_type::unsupported)
    {
      ++m_testcase_failures;
    }
  }
...
#  pragma GCC diagnostic pop
  ...
  void print_summary(const char* suitename) const
  {
    ...
    std::fprintf(out,
                 ...
                 m_assertions - (m_warning_failures + m_check_failures + m_require_failures),
                 m_assertions);
...
public:
  ...
  {
    // for each testcase
    for (test_suite::const_iterator b = m_ts.begin(), e = m_ts.end(); b != e; ++b)
    {
...
bool check_equal_collections_impl(Iter1 start1, Iter1 const end1, Iter2 start2, Iter2 const end2)
{
