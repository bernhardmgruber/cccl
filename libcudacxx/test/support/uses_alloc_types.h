...
inline bool CompareVerbose(const char* LHSString, UsesAllocatorType LHS, const char* RHSString, UsesAllocatorType RHS)
{
  if (LHS == RHS)
  {
    return true;
...
struct TakeNImp<false, N, ArgumentListID<A1...>, F, R...> : TakeNImp<N - 1 == 0, N - 1, ArgumentListID<A1..., F>, R...>
{};
...
struct UsesAllocatorTestBaseStorage
{
  ...
  UsesAllocatorTestBaseStorage(Alloc const& a)
      : allocator(a)
  {}
...
public:
  ...
  bool checkConstruct(UsesAllocatorType expectType) const
  {
    ...
    return COMPARE_ALLOC_TYPE(expectType, constructor_called) && COMPARE_TYPEID(args_id, expectArgs);
  }
  ...
  bool checkConstruct(UsesAllocatorType expectType, CtorAlloc const& expectAlloc) const
  {
    ...
    return COMPARE_ALLOC_TYPE(expectType, constructor_called) && COMPARE_TYPEID(args_id, ExpectID) && has_alloc()
        && expectAlloc == *get_alloc();
  }
  ...
  bool checkConstructEquiv(UsesAllocatorTestBase& O) const
  {
    ...
    return COMPARE_ALLOC_TYPE(constructor_called, O.constructor_called) && COMPARE_TYPEID(args_id, O.args_id)
...
protected:
  ...
  UsesAllocatorTestBase(UsesAllocatorTestBase const&)
      : args_id(&makeArgumentID<Self const&>())
      , constructor_called(UA_None)
      , alloc_store()
  {}

  UsesAllocatorTestBase(UsesAllocatorTestBase&&)
      : args_id(&makeArgumentID<Self&&>())
      , constructor_called(UA_None)
      , alloc_store()
  {}
  ...
  UsesAllocatorTestBase(AllocLastTag, Args&&... args)
      ...
      , alloc_store(UsesAllocatorTestBase::getAllocatorFromPack(typename ArgsIDL::type{}, std::forward<Args>(args)...))
  {}
...
private:
  ...
  {
    return alloc_store.get_allocator() != nullptr;
  }
  const CtorAlloc* get_alloc() const
  {
    return alloc_store.get_allocator();
  }
...
public:
  ...
  UsesAllocatorV1(Args&&...)
      : Base(&makeArgumentID<Args&&...>())
  {}
