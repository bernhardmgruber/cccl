  ...
  {
    {
      ...
      assert(cuda::std::atomic_fetch_xor_explicit(&t, T(2), cuda::std::memory_order_seq_cst) == T(1));
      assert(t == T(3));
    }
    {
      using A = cuda::std::atomic<T>;
      ...
      assert(cuda::std::atomic_fetch_xor_explicit(&t, T(2), cuda::std::memory_order_seq_cst) == T(3));
      assert(t == T(1));
    }
  }
};
