  ...
  {
    {
      ...
      assert(cuda::std::atomic_fetch_and_explicit(&t, T(2), cuda::std::memory_order_seq_cst) == T(1));
      assert(t == T(0));
    }
    {
      using A = cuda::std::atomic<T>;
      ...
      assert(cuda::std::atomic_fetch_and_explicit(&t, T(2), cuda::std::memory_order_seq_cst) == T(3));
      assert(t == T(2));
    }
  }
};
