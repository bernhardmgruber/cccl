  ...
  {
    {
      ...
      assert(cuda::std::atomic_fetch_sub_explicit(&t, T(2), cuda::std::memory_order_seq_cst) == T(3));
      assert(t == T(1));
    }
    {
      using A = cuda::std::atomic<T>;
...
{
  {
    ...
    assert(cuda::std::atomic_fetch_sub_explicit(&t, 2, cuda::std::memory_order_seq_cst) == T(3 * sizeof(X)));
