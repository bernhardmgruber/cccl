...
struct make_zip_iterator_base<::cuda::std::tuple<Its...>>
{
  ...
  using reference = tuple_of_iterator_references<iter_reference_t<Its>...>;

  // Boost's Value type is the same as reference type. using value_type = reference;
  using value_type = ::cuda::std::tuple<iter_value_t<Its>...>;

  // Difference type is the first iterator's difference type
  using difference_type = iter_difference_t<::cuda::std::tuple_element_t<0, ::cuda::std::tuple<Its...>>>;
