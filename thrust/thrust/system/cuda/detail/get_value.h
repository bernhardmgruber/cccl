...
_CCCL_HOST_DEVICE thrust::detail::iter_value_t<Pointer> get_value(execution_policy<DerivedPolicy>& exec, Pointer ptr)
{
  ...
  struct HostPath
  {
    _CCCL_HOST auto operator()(execution_policy<DerivedPolicy>& exec, Pointer ptr)
    {
      ...
      thrust::detail::iter_value_t<Pointer> result;
